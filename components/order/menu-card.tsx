...
import { Plus, Minus, Check, Maximize2 } from 'lucide-react';
import { useCartStore } from '@/lib/store/cart-store';
import { ItemDetailModal } from '@/components/order/item-detail-modal';
  ...
  return (
    <>
      <div className="menu-card group !p-3 sm:!p-4">
        {/* Image Thumbnail Clickable Target */}
        <div
          onClick={() => setIsDetailOpen(true)}
          className="relative w-20 h-20 sm:w-24 sm:h-24 rounded-xl overflow-hidden bg-slate-800 flex-shrink-0 cursor-pointer group/img"
        ...
        >
          ...
          <div className="absolute inset-0 bg-slate-950/40 opacity-0 group-hover/img:opacity-100 transition-opacity flex items-center justify-center">
            <span className="bg-slate-950/70 text-amber-400 p-1.5 rounded-full border border-amber-500/30 backdrop-blur-sm">
              <Maximize2 size={12} />
            </span>
          </div>
        ...
        <div className="flex-1 min-w-0 flex flex-col justify-between self-stretch py-0.5">
          <div onClick={() => setIsDetailOpen(true)} className="cursor-pointer">
            <div className="flex items-start justify-between gap-1.5">
              <h3 className="font-bold text-slate-100 text-sm sm:text-base leading-snug font-display truncate hover:text-amber-400 transition-colors">
                {item.name}
              </h3>
      ...
      {/* Detailed Large Image Modal Popup */}
      <ItemDetailModal
        item={item}
        isOpen={isDetailOpen}
